...
def require_keys(label: str, value: dict, required: set[str]) -> list[str]:
...
def main() -> int:
    ...
    missing = require_keys("mapping", mapping, REQUIRED_TOP)
    if missing:
    ...
    if not isinstance(spec, dict):
        ...
        return 1
    missing = require_keys("mapping spec", spec, REQUIRED_SPEC)
    if missing:
    ...
    missing = require_keys("entity_mappings", entity_mappings, REQUIRED_ENTITY_KEYS)
    if missing:
        print(f"[FAIL] entity_mappings missing: {', '.join(missing)}", file=sys.stderr)
        failed = True
    missing = require_keys("trader_agent_mappings", trader_mappings, REQUIRED_TRADER_KEYS)
    if missing:
